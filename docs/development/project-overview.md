...
### v0.4 — 検索の性能改善とセキュリティ監査

#### 性能改善 (v0.3 から移動)

- `search_page_match()` が候補ページごとに `wiki_texts()` でフルパースしている。
  キャッシュは無い。候補 1 件あたり 1.4〜2.4ms で、**ページ数に線形**
- 1 文字の検索が最悪ケース。索引でほとんど絞れず、ほぼ全ページが候補になる
  (1372 ページのサイトで `a` は 1167 件が候補)
- 方式の候補: テキスト抽出結果をキャッシュする / 索引側に本文を持たせる。
  どちらも保存時のコストと storage の使用量に跳ねるので、決めてから着手する

#### セキュリティ監査
- XSS / CSRF / パストラバーサル / ファイルアップロード
- MD5 認証ダイジェストの見直し
- ~~**`storage/` のディレクトリ一覧が外部から見える**~~ → **v0.3.1 で修正済み**。
  配布する `.htaccess` に `Options -Indexes` を追加した。
  既存サイトの `.htaccess` は書き換えないので (利用者が手を入れていることがある)、
  手順書に手作業の項目として書き、アップグレードツールが不足を検出して知らせる。
