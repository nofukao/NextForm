...
| テーマを触る変更の担保 (追記 2026-08-18) | **設定パターンを変えながら生成物を全部吐き、リファクタ前後で diff を取る** | ゴールデンマスターは HTML しか見ず、`css-rules.sh` は名指しした数点しか見ない。共通化のような機械的な移動は「1 バイトも変わらない」ことで担保するのが最も確実で、実際に 16 パターンで確認した |
