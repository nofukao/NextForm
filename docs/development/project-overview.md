...
### v0.4.0 — CSRF 対策 ✅ 完了 (2026-08-17)

セキュリティ監査の結果、**データが消える性質の穴が 1 つ**見つかったので、
性能改善より先に出すことにした (v0.3.0 と同じ判断)。
...
- **realm が全サイト共通の固定値** `'User login'`。保存されるのは
  `md5(利用者名:'User login':パスワード)` で、realm がソルトとして働かない。
  **NextForm 全サイトで同じ事前計算表が通用する。**
  変えると保存済みダイジェストが全部無効になり、全利用者のパスワード
  再設定が必要になるため、単独で計画する
- **インストール後も `index.php` が Web サーバから書ける** (`-rw-r--r-- apache apache`)。
  インストーラが `index.php` を書き換える設計のため。書き込み口が 1 つ
  見つかると RCE に直結する
- **`&link()` の URI 判定が前方一致でない。** `WIKI_URI_REGEX` がアンカーされて
  おらず、一致すると値全体を `href` にする (`&link(abchttp://example.com/);` →
  `href="abchttp://example.com/"`)。ただし `javascript:` は `:` が
  パラメータ解析で落ちるため**注入できなかった**(複数パターンで実測)。
  実害は確認できていないが、アンカー + スキーム許可リストにしたい
- **SSRF。** `&include_uri` / `getimagesize()` が任意の URL を取りに行く。
  書き込み権限が必要なので優先度は低い
